...
var _speed: int
var _last_collider: String

# Handles when the speed_increase animation will play
var _play_speed_increase: bool
...
@onready var _paddle_hit_effect: CPUParticles2D = $PaddleHitEffect
@onready var _ball_hit_effect: CPUParticles2D = $BallHitEffect
@onready var _speed_effect: CPUParticles2D = $SpeedEffect

@onready var _boundary_hit_sound: AudioStreamPlayer2D = $BoundaryHitSound
@onready var _paddle_hit_sound: AudioStreamPlayer2D = $PaddleHitSound
@onready var _paddle_hit_sound_2: AudioStreamPlayer2D = $PaddleHitSound2
@onready var _paddle_hit_sound_3: AudioStreamPlayer2D = $PaddleHitSound3
@onready var _timer_sound: AudioStreamPlayer2D = $TimerSound
@onready var _go_sound: AudioStreamPlayer2D = $GoSound

@onready var _ball_movement_timer: Timer = $BallMovementTimer
@onready var _count_down_sound_timer: Timer = $CountDownSoundTimer

var _time_not_up : bool

func _ready():
	
	# Adds a fade in transition at the start of the scene
...
func start_ball():
	...
	var _win_length: float = get_viewport().get_visible_rect().size.x
	var _win_height: float = get_viewport().get_visible_rect().size.y
	
	# Center ball along the x axis
	position.x = (_win_length / 2.0) - (_color_rect.size.x / 2.0)
	
	# Set ball along the y axis randomly
	position.y = randf_range(_color_rect.size.y / 2.0, _win_height - _color_rect.size.y / 2.0)
...
func _random_direction(_last_scorer: String) -> Vector2:
...
func _direction_after_paddle(_current_collider: Object) -> Vector2:
	
	# Get paddle center and distance between ball and paddle center
	var _paddle: ColorRect = _current_collider.get_node("ColorRect")
	var _paddle_center: float = _paddle.get_size().y / 2.0
	var _distance_from_center: float = position.y - (_paddle_center + _paddle.global_position.y)
...
func _handle_collision_effects():
	...
	if _speed < 800:
		...
		# Configures the effects of the ball hit effect
	...
	# If the speed is greater than 800
	else:
		...
		# Congigures effects for the ball hit effect based on speed increase
		_ball_hit_effect.initial_velocity_max = 100
		_ball_hit_effect.lifetime = 0.6
		
		# Determines the direction to emit particles when the ball was hit by the CPU
		if _last_collider == "CPU":
			# Set the particles effect to the right side of the ball
			_ball_hit_effect.position.x = 38
...
# Plays the speed_increase animation and effectwhen the speed of the ball is over 800 and _play_speed_increase is true
func _play_speed_increase_vfx():
	
	if _speed > 800 && _play_speed_increase == true:
		
		#Plays the speed_increase animation
		_animation_player.play("speed_increase")
